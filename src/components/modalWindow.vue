  ...
  <div class="modal-window">
    <Form @submit="onSubmit" v-slot="{ errorBag, meta }">
      ...
      <div class="descr-form">
        Успех начинается с правильных решений. Оставьте здесь свои контакты, и мы свяжемся с вами,
        чтобы предложить оптимальную технологию по увеличению процента извлекаемости полезного в
      ...
      <Field
        ...
        type="email"
        placeholder="E-Mail*"
        :class="{ 'has-error': errorBag.email }"
        :rules="validateEmail"
      />
      ...
      <Field
        ...
        :class="{ 'has-error': errorBag.description }"
        :rules="required"
      />
      <label class="wrapper-input" :class="{ 'has-error': errorBag.file }">
        <Field
          ...
          :rules="{ fileValidation: true, size: true, image: true }"
        >
          <input
            type="file"
            @change="handleChange"
            @blur="handleBlur"
            class="input__file"
            placeholder="приложить файлы (до трех файлов)"
            multiple
            ref="fileInput"
          />
        </Field>
      ...
      <div>
        Отправляя эту форму, я принимаю <a href="">политика конфиденциальности</a> этого сайта.
      </div>
